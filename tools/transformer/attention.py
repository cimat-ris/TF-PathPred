...
class Multi_headed_attention_cvae(tf.keras.layers.Layer):
  def __init__(self, d_model, num_heads, num_modes):
    super(Multi_headed_attention_cvae, self).__init__()
    #Number of modes
  ...
  def call(self, v, k, q, mask):
    batch_size    = tf.shape(q)[0]
    num_modes     = tf.shape(q)[1]
    sequence_size = tf.shape(q)[2]
    # All inputs size: num_batch x num_modes x sequence_length x d_model
    # Generate the query, key, value vectors
    q = self.wq(q)
    k = self.wk(k)
    v = self.wv(v)
    ...
    k = self.split_heads(k)
    v = self.split_heads(v)
    # Size q,k,v: num_batch x num_modes x num_heads x sequence_length x depth
    scaled_attention, attention_weights = scaled_dot_product_attention(q, k, v, mask)
    # concat_attention = tf.reshape(scaled_attention,[scaled_attention.shape[0],-1,self.d_model])
    concat_attention = tf.reshape(scaled_attention,[batch_size,num_modes,sequence_size,self.d_model])
    # Size output: num_batch x num_modes x sequence_length x d_model
    output = concat_attention
    return output, attention_weights